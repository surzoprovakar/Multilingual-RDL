...
public class Server {
    ...
    private static MapCRDT mapReplica;
    ...
    public static void DoActions(List<String> actions) throws InterruptedException, IOException {
        ...
        for (String action : actions) {
            if (action.contains(":")) {
                String[] reqs = action.split(":");
                String opt = reqs[0];
                String key = reqs[1];
                if ("Add".equals(opt)) {
                    int val = Integer.parseInt(reqs[2]);
                    mapReplica.Add(key, val);
                    mapReplica.Print();
                ...
                System.out.println("About to broadcast Map");
                mapReplica.Print();
                Client.Broadcast(conns, mapReplica.ToMarshal());
            } else {
                ...
                while ((bytesRead = input.read(buffer)) != -1) {
                    for (int i = 0; i < bytesRead; i++) {
                        ...
                        if (b == 0x00) { // delimiter
                            ...
                            Pair<Integer, List<String>> req = MapCRDT.FromMarshalData(message);
                            mapReplica.Merge(req.first, req.second);
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        if (args.length != 4) {
            System.out.println("Usage: map_id ip_address crdt_socket_server Replicas'_Addresses.txt Actions.txt");
            System.exit(1);
        }
