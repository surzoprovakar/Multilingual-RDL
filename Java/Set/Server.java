...
public class Server {
    ...
    private static SetCRDT setReplica;
    ...
    public static void DoActions(List<String> actions) throws InterruptedException, IOException {
        ...
        for (String action : actions) {
            if (action.contains(":")) {
                ...
                System.out.println("About to broadcast Set");
                setReplica.Print();
                Client.Broadcast(conns, setReplica.ToMarshal());
            } else {
                ...
                while ((bytesRead = input.read(buffer)) != -1) {
                    for (int i = 0; i < bytesRead; i++) {
                        ...
                        if (b == 0x00) { // delimiter
                            ...
                            Pair<Integer, List<String>> req = SetCRDT.FromMarshalData(message);
                            setReplica.Merge(req.first, req.second);
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        if (args.length != 4) {
            System.out.println("Usage: set_id ip_address crdt_socket_server Replicas'_Addresses.txt Actions.txt");
            System.exit(1);
        }
