...
func do_actions(actions []string) {
	...
	for _, action := range actions {
		if action == "Inc" {
			counterReplica.Inc()
			fmt.Println(counterReplica.Print())
		} else if action == "Dec" {
			counterReplica.Dec()
			fmt.Println(counterReplica.Print())
		} else if action == "Broadcast" {
			// // func before boradcast
			// b1 := counterReplica.ToMarshal()
			...
			//conns = establishConnections(hosts)
			fmt.Println("About to broadcast Counter" + counterReplica.Print())
			broadcast(conns, counterReplica.ToMarshal())
		} else if strings.Contains(action, "#") {
			// do nothing, consider as a comment in action script
		} else if strings.Contains(action, "Undo") || strings.Contains(action, "Rev") {
			counterReplica.Call_Plugin(counterReplica.id, action)
			// intentionally waiting 5s so that the correspoding return action gets enough time to arrive
			time.Sleep(time.Duration(5) * time.Second)
			// if or not after notify other replicas
			broadcast(conns, counterReplica.ToMarshal())
...
func handleConnection(conn net.Conn) {
	...
	*/
	...
	// fmt.Println(updates)
	counterReplica.Merge(rid, updates)
