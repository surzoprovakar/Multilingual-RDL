...
func do_actions(actions []string) {
	...
	for _, action := range actions {
		if strings.Contains(action, ":") {
			...
			opt := action_data[0]
			value, _ := strconv.Atoi(action_data[1])
			if opt == "Add" {
				setReplica.Add(value)
				setReplica.Print()
			} else if opt == "Remove" {
				setReplica.Remove(value)
				setReplica.Print()
			}
		} else if action == "Broadcast" {
			// // func before boradcast
			// b1 := setReplica.ToMarshal()
			...
			//conns = establishConnections(hosts)
			fmt.Println("About to broadcast Set")
			setReplica.Print()
...
func main() {
	...
	input := os.Args[1:]
	if len(input) != 4 {
		println("Usage: set_id ip_address crdt_socket_server Replicas'_Addresses.txt Actions.txt")
		os.Exit(1)
	}
