...
async function doActions(actions) {
    ...
    for (const action of actions) {
        if (action === "Inc") {
            await counterReplica.inc()
            console.log(counterReplica.print())
        } else if (action === "Dec") {
            await counterReplica.dec()
            console.log(counterReplica.print())
            ...
            console.log(`About to broadcast Counter: ${counterReplica.print()}`)
            // console.log("broadcast content: ", counterReplica.toMarshal())
...
function handleConnection(conn) {
    ...
    function processMessage(message) {
        ...
        const [rid, updates] = Counter.fromMarshalData(messageUint8Array)
        // console.log("rid: ", rid)
